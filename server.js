...
// ─── Passkey login ───
app.post('/api/auth/passkey/login-options', loginLimiter, async (req, res) => {
  try {
...
app.post('/api/auth/passkey/verify-2fa', loginLimiter, async (req, res) => {
  try {
...
// ─── Passkey 2FA options (for pending 2FA) ───
app.post('/api/auth/passkey/2fa-options', loginLimiter, async (req, res) => {
  try {
    if (!req.session.pending2faUserId) return res.status(400).json({ error: 'No pending 2FA verification' });
    const { rpID } = getWebAuthnConfig(req);
...
app.delete('/api/auth/passkey/credentials/:id', requireAuth, validateId, async (req, res) => {
  try {
    // Check ownership
    ...
    // If no more passkeys, reset mode to 'either'
...
app.put('/api/auth/passkey/mode', requireAuth, async (req, res) => {
  try {
    const { mode } = req.body;
    ...
    if (mode === 'passwordless') {
      ...
      // Allow passwordless even without password, as long as passkeys exist
    }
    if (mode === 'twofactor') {
      const user = await query('SELECT password_hash FROM users WHERE id = $1', [req.session.userId]);
...
app.delete('/api/auth/google/link', requireAuth, async (req, res) => {
  try {
    const user = await query('SELECT password_hash FROM users WHERE id = $1', [req.session.userId]);
    const credCount = await query('SELECT COUNT(*) as count FROM user_credentials WHERE user_id = $1', [req.session.userId]);
...
app.put('/api/settings/google-auth', requireAdmin, async (req, res) => {
  try {
